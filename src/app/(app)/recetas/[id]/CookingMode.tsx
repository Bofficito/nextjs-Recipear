...
import { X, ChevronLeft, ChevronRight, ChefHat, Timer, Play, Square, Pause } from 'lucide-react'
...
function StepTimer() {
  ...
  if (done) return (
    <div className="flex items-center gap-3 bg-green-950 border border-green-800 rounded-xl px-4 py-3">
      ...
      <button onClick={stop} className="ml-auto text-green-600 hover:text-green-400 transition-colors">
        <X size={14} />
      </button>
    </div>
  )
  ...
  if (running && seconds !== null) return (
    <div className={`flex items-center gap-3 border rounded-xl px-4 py-2.5 transition-colors ${
      seconds <= 10 ? 'bg-red-950 border-red-800' : 'bg-stone-900 border-stone-700'
    }`}>
      <Timer size={14} className={seconds <= 10 ? 'text-red-400' : 'text-stone-400'} />
      <span className={`font-mono text-sm tabular-nums ${seconds <= 10 ? 'text-red-400' : 'text-white'}`}>
        {formatTime(seconds)}
      </span>
      <button onClick={stop} className="ml-auto text-stone-600 hover:text-white transition-colors">
        <Square size={14} />
      </button>
    </div>
  )
          ...
          {parsedSteps.map((_, i) => (
            <button
              ...
              className={`h-1 rounded-full transition-all duration-300 ${
                i === current ? 'bg-white w-8' : i < current ? 'bg-stone-600 w-4' : 'bg-stone-800 w-4'
              }`}
            />
          ))}
        </div>
