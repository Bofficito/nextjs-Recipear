  ...
  return (
    <Document>
      ...
      <Page size="A4" style={s.coverPage}>
        ...
        <Text style={s.coverCount}>{recipes.length} receta{recipes.length !== 1 ? 's' : ''}</Text>
      </Page>
      ...
      <Page size="A4" style={s.page}>
        ...
        {recipes.map((r, i) => (
          <View key={r.id} style={s.indexEntry}>
            <Text style={s.indexTitle}>{i + 1}. {r.title}</Text>
            <Text style={s.indexMeta}>{r.category}{r.time ? ` · ${r.time}` : ''}</Text>
          </View>
        ))}
      ...
      {recipes.map((recipe, index) => {
        ...
        return (
          <Page key={recipe.id} size="A4" style={s.page}>
            ...
            <View style={s.footer} fixed>
              <Text style={s.footerText}>mis recetas</Text>
              <Text style={s.footerText}>{index + 1} / {recipes.length}</Text>
            </View>
          </Page>
        )
      })}
